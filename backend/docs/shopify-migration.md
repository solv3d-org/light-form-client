...
- Never commit `.env`, Shopify client secrets, Admin tokens, or Storefront tokens.
...
6. Add them to `.env` as `SHOPIFY_CLIENT_ID` and `SHOPIFY_CLIENT_SECRET`.
...
Run from backend repo root after `.env` is configured:
...
- Wrong store domain/subdomain in `.env`.
